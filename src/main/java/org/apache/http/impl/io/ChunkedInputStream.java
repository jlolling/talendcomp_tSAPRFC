    ...
    private long getChunkSize() throws IOException {
        ...
        case CHUNK_CRLF:
        	...
        	if (this.buffer != null && this.buffer.length() > 0) {
            	lastBufferContent = String.valueOf(this.buffer);
            	lastBufferContent = StringEscapeUtils.escapeJava(lastBufferContent);
        	}
            this.buffer.clear();
            final int bytesRead1 = this.in.readLine(this.buffer);
            if (bytesRead1 == -1) {
                throw new MalformedChunkCodingException(
                    "CRLF expected at end of chunk. Last buffer content: " + lastBufferContent);
            }
